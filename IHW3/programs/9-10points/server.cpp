...
onlyfast::network::Response echo_handler(const onlyfast::Application::RequestData &rd)
{
    ...
    return {onlyfast::network::ResponseStatus::OK, rd.params[0]};
}

onlyfast::network::Response register_programmer_handler(const onlyfast::Application::RequestData &rd)
{
    if (rd.params.empty())
    {
        return {onlyfast::network::ResponseStatus::FAILED, "No parameters"};
    }
    int id = solution.add_programmer(rd.params[0]);
    return {onlyfast::network::ResponseStatus::OK, std::to_string(id)};
}

onlyfast::network::Response take_job_handler(const onlyfast::Application::RequestData &rd)
{
    if (rd.params.empty())
    {
        return {onlyfast::network::ResponseStatus::FAILED, "No parameters"};
    }
    return {onlyfast::network::ResponseStatus::OK, solution.take_job(rd.params[0])};
}

onlyfast::network::Response send_on_check_handler(const onlyfast::Application::RequestData &rd)
{
    if (rd.params.empty())
    {
        return {onlyfast::network::ResponseStatus::FAILED, "No parameters"};
    }
    solution.send_on_check(rd.params[0], rd.params[1]);
    return {onlyfast::network::ResponseStatus::OK, "OK"};
}

onlyfast::network::Response send_check_result_handler(const onlyfast::Application::RequestData &rd)
{
    if (rd.params.empty())
    {
        return {onlyfast::network::ResponseStatus::FAILED, "No parameters"};
    }
    solution.send_check_result(rd.params[0], rd.params[1], rd.params[2]);
    return {onlyfast::network::ResponseStatus::OK, "OK"};
}
...
int main(int argc, char **argv)
{
    ...
    app = new onlyfast::Application(server);
